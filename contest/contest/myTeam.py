...
class OffensiveAgent(CaptureAgent):
  ...
  #for n,w in {"pac-dist-to-opp": 0.0,"len-eat-food": -1.0, "pac-dist-to-food": 0.0}.items():
      #weights[n] = w
  epsilon = 0.05
  alpha = 0.02
  discount = 0.8
...
#
  ...
  def getReward(self, gameState):
      ...
      if gameState.getAgentState(self.index).isPacman == False:
          return 1.0 / (float)( self.getMazeDistance(gameState.getAgentPosition(self.index), foodChoice ) + gameState.getScore())
      else:
          closestGhostDistance = self.MAX_VALUE
          ...
          if closestGhostDistance == 0.0 or closestGhostDistance > 3:
              ...
              closestGhostDistance = 1.0 / float( closestGhostDistance )
          return ( 1/len(eatFood) + closestGhostDistance )
